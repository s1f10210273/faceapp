...
import { Box, Flex, Text, Image, Alert, AlertIcon, AlertTitle, AlertDescription, VStack, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, useDisclosure, Button } from '@chakra-ui/react';
  ...
  const sendImage = async (imageSrc: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setResponseData({ image: imageSrc, message: 'Error occurred', predicted_age: 55 });
      } else {
      ...
      setTimeout(() => {
        onClose();
        if (responseData?.predicted_age! > 60) {
          router.push('/over60');
        } else {
          router.push('/young');
        }
